Read `.redlining/annotations.md`. If `.redlining/screenshot.png` exists, view it — pins are numbered like the annotations. An annotation's `Reference` line names a pasted mockup of the intended result and its `Crop` line the element as it looks now; view both before editing.
