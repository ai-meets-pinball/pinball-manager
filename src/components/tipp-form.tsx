...
/*
  Neuen allgemeinen Tipp anlegen (typ='tipp') — als eigene Seite
  (/machines/[id]/tipps/new, Redlining 2026-09-13), nicht mehr als Dialog.
  Der Geltungsbereich ist n:m: beliebig viele Modelle und/oder Generationen
  aus dem Katalog.

  Die Ziel-Auswahl ist gestuft, vom Nahen zum Fernen: erst DIESES Gerät
  (vorausgewählt), dann weitere Editionen desselben Titels (z. B. Pro ↔
  Premium/LE), dann — zugeklappt — andere Modelle, und getrennt davon ganze
  Generationen. Die Ordnung liefert lib/tipp-ziele.
  ...
  dem Submit verlieren. Nach Erfolg (state.ok) geht es zurück zum Tipps-Reiter.
*/
type Katalog = {
  machineId: string;
  modelle: {
  ...
  /** Ziel nach dem Speichern bzw. für „Abbrechen" (der Tipps-Reiter). */
  zurueck: string;
};
...
export function TippFormular({
  ...
  useEffect(() => {
    if (state.ok) router.push(zurueck);
  }, [state.ok, router, zurueck]);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <Field
        label="Tipp"
        hint="Formatierung über die Knöpfe oder direkt: **fett**, _kursiv_, Aufzählung mit Bindestrich am Zeilenanfang, [Text](https://…). URLs werden automatisch anklickbar."
      >
        <FormatTextarea
          name="text"
          required
